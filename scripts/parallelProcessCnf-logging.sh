...
#结果文件的位置
processed_files="/home/wgf/chenli/SAT/cadical-2.0.0-kalman/2022result.csv"
#处理的cnf文件个数
total_files=400
...
head -n $total_files /tmp/unprocessed_files.txt | xargs -n 1 -P $num_cores -I {} bash -c '
    ...
    timeout 3600 /home/wgf/chenli/SAT/cadical-2.0.0-kalman/build/cadical -l "$str" | awk -F "[{}]" "/statistics/{flag1=1;next} flag1{print \$0; if(++n1==29) flag1=0} /resources/{flag2=1;next} flag2{print \$0; if(++n2==6) exit}" >> "$temp_file"
    printf "\n" >> "$temp_file"
    echo "end $str"
